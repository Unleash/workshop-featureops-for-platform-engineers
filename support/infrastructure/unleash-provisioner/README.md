...
- **Segment** — the project-scoped `pNNN_internal-users` segment (`pNNN_email` ends with `@getunleash.io`).
...
Optional overrides: `UNLEASH_ENVIRONMENTS` (default `development production`), `UNLEASH_CR_ENVIRONMENTS` (default `production`), `UNLEASH_PROD_REQUIRED_APPROVALS` (default `1`).
