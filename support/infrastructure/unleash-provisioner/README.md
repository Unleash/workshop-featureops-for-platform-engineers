...
**Self-paced** (`UNLEASH_SELF_PACED=1`, via `make workshop-provision`) — one attendee, one project, **no Terraform**. On top of the above, this tool takes over Terraform's job: it **creates the project** (enabling the instance's existing `development` and `production` environments on it — a fresh project has none enabled), and applies the naming pattern and links. Because the attendee owns their whole instance, names are **not prefixed**: the segment is simply `internal-users`. Two things are deliberately skipped: archiving the built-in `Default` project (destructive on an instance we don't own) and the master kill switch (its Action needs a Terraform-provisioned service account). On the way out (`make workshop-teardown`), `destroy` deletes the project itself too — there is no Terraform to do it.

`destroy` removes every flag in a project — the workshop's four and any the attendee created — in the two steps Unleash requires: archive, then delete from the archive (which also frees the names).
