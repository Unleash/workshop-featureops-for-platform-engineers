...
- [ ] Confirm the app emits the impact metric by creating an _Impact Metrics_ chart.
  - The checkout SDK pushes `<prefix>checkout_error_total` (and related provider-error counters) to the _Unleash_ instance.
- [ ] On your flag's release plan, add a **safeguard** that references your metric.
  - Use a tight, easy-to-trip override — e.g. **count of `<prefix>checkout_error_total` > 0 in the last minute** (the short evaluation window will make it fire quickly).
