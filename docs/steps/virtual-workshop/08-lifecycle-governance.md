...
Flag guarding the _promotion code_ functionality has completed its lifecycle. After the cleanup, the code path is clean, with the archive recorded in the audit log (which you can verify under this _URL_: `https://<REGION>.app.unleash-hosted.com/<INSTANCE_ID>/projects/<project-NNN>/logs`).
