...
import {
  ...
  SESSION_INIT_FAILURE_RATE,
  occursWithProbability,
} from './support/failure-injection';
...
const main = async (): Promise<void> => {
  ...
  const notifyMerchant = async (payment: Payment): Promise<void> => {
    const response = await fetch(payment.webhookUrl, {
      ...
      headers: {
        ...
        'x-dashed-signature': config.webhookSecret,
      },
      body: JSON.stringify({
  ...
  const app = buildApp(config, {
    notifyMerchant,
    shouldFailCreate: () => occursWithProbability(SESSION_INIT_FAILURE_RATE),
    shouldFailConfirm: () => occursWithProbability(CAPTURE_FAILURE_RATE),
  });
