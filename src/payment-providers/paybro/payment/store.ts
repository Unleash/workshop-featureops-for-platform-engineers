...
/** The merchant supplies everything except what PayBro derives (id, charged amount, status, code). */
export type NewPayment = Omit<Payment, 'paymentId' | 'amountChargedCents' | 'status' | 'errorCode'>;

const payments = new Map<string, Payment>();

// PayBro is the authority on what it charges: development is a sandbox, so no real
